...
import { Check, QrCode, Heart, ArrowRight, X, Star, Sparkles, Moon, Copy, Wallet } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function PaymentCards({ paymentInfo }: PaymentCardsProps) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      ...
      <div className="text-center mb-8 opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.6s" }}>
        <div className="relative inline-block">
          ...
          <div className="relative flex items-center gap-3 px-6 py-3 bg-background/30 backdrop-blur-xl rounded-2xl border border-primary/20">
            ...
            <span className="text-base font-bold text-foreground">পেমেন্ট মাধ্যম নির্বাচন করুন</span>
      ...
      <div className="flex md:grid md:grid-cols-3 gap-5 overflow-x-auto pb-4 md:pb-0 snap-x snap-mandatory md:snap-none scrollbar-hide px-2 md:px-0">
        {paymentMethods.map((method, index) => (
          <div
            key={method.id}
            className={`flex-shrink-0 w-[85vw] md:w-auto snap-center opacity-0 animate-slide-up fill-forwards`}
            style={{ animationDelay: mounted ? `${0.7 + index * 0.15}s` : '0s' }}
          >
            <div
              ...
              <div className="relative z-10 p-6">
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <div className="relative mb-5">
                    <div className={`absolute inset-[-8px] bg-gradient-to-r ${method.color} rounded-full opacity-30 blur-xl animate-breathe`} />
                    <div className={`w-20 h-20 rounded-2xl bg-white flex items-center justify-center shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 p-3 relative`}>
                  ...
                  {/* Logo Badge */}
                  <div className={`px-5 py-2 rounded-xl bg-gradient-to-r ${method.color} text-white text-sm font-bold shadow-lg mb-4 group-hover:scale-105 transition-transform duration-300`}>
                    {method.logo}
                  </div>

                  {/* Method Name */}
                  <h4 className={`text-2xl font-black ${method.textColor} mb-2 group-hover:scale-105 transition-transform duration-300`}>
                    {method.name}
                  </h4>
                  <p className="text-sm text-foreground/60 mb-6">দিয়ে ঈদি পাঠান</p>

                  {/* Features */}
                  <div className="flex items-center justify-center gap-4 mb-6">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-background/50 border ${method.borderColor} text-xs font-medium`}>
                      <QrCode className={`h-3.5 w-3.5 ${method.textColor}`} />
                      <span className="text-foreground/70">QR কোড</span>
                    </div>
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-background/50 border ${method.borderColor} text-xs font-medium`}>
                      <Heart className={`h-3.5 w-3.5 ${method.textColor} fill-current`} />
            ...
            <>
                ...
                <DialogHeader className="relative z-10">
                  <DialogTitle className="text-white text-center flex flex-col items-center gap-3">
                    ...
                    <div className="opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.1s" }}>
                      <span className="text-2xl font-bold block">{instructionModal.method.name}</span>
                      <span className="text-white/80 text-sm">দিয়ে ঈদি পাঠান</span>
                    </div>
                  </DialogTitle>
              ...
              <div className="p-6 space-y-5 relative bg-background/40 backdrop-blur-md">
                ...
                <div className="absolute top-4 right-4 opacity-10 animate-spin-slow">
                  <Moon className={`h-12 w-12 ${instructionModal.method.textColor}`} />
                </div>
                ...
                <div className="space-y-3 opacity-0 animate-slide-up fill-forwards relative z-10" style={{ animationDelay: "0.15s" }}>
                  <h5 className="font-bold text-foreground text-sm flex items-center gap-2">
                    <Sparkles className={`h-4 w-4 ${instructionModal.method.textColor} animate-spin-slow`} />
                    পেমেন্ট নির্দেশনা
                  </h5>
                  <ol className="space-y-2">
                    {instructionModal.method.steps.map((step, index) => (
                      ...
                      >
                        <span className={`flex-shrink-0 w-6 h-6 rounded-lg bg-gradient-to-r ${instructionModal.method!.color} text-white text-xs font-bold flex items-center justify-center animate-scale-pulse`}>
                          {index + 1}
                        </span>
                ...
                <div className={`p-5 rounded-2xl border ${instructionModal.method.borderColor} bg-background/30 backdrop-blur-lg relative overflow-hidden opacity-0 animate-slide-up fill-forwards`} style={{ animationDelay: "0.5s" }}>
                  {/* Animated corner decorations */}
                  <div className="absolute top-2 right-2 animate-twinkle-star">
                    <Star className={`h-3 w-3 ${instructionModal.method.textColor} opacity-40`} />
                  </div>
                  ...
                  >
                    <span className={`font-mono text-2xl tracking-wider font-bold ${instructionModal.method.textColor}`}>
                      {getNumber(instructionModal.method.id)}
                    </span>
                    <div className={`p-2.5 rounded-xl transition-all duration-300 ${copiedId === `modal-${instructionModal.method.id}` ? 'bg-green-500/30' : `bg-gradient-to-r ${instructionModal.method.color}`}`}>
                  ...
                  {copiedId === `modal-${instructionModal.method.id}` && (
                    <p className="text-center text-green-500 text-xs mt-2 animate-slide-up font-medium">কপি হয়েছে!</p>
                  )}
                </div>

                {/* QR Code */}
                {getQRCode(instructionModal.method.id) && (
                  <div className="flex flex-col items-center opacity-0 animate-slide-up fill-forwards rounded-2xl bg-background/20 backdrop-blur-lg p-5 border border-border/20 relative" style={{ animationDelay: "0.6s" }}>
                    {/* Animated decorations around QR */}
                    <div className="absolute top-3 left-3 animate-float-gentle">
                      <Sparkles className={`h-4 w-4 ${instructionModal.method.textColor} opacity-30`} />
                    </div>
      ...
      <Dialog open={showThankYou} onOpenChange={setShowThankYou}>
        <DialogContent className="bg-background/60 backdrop-blur-2xl border-primary/20 max-w-md text-center overflow-hidden p-0 rounded-3xl [&>button]:hidden animate-scale-in shadow-2xl" aria-describedby="thank-you-description">
          ...
          <div className="px-8 py-10 space-y-6 relative z-10">
            ...
            <div className="relative mx-auto w-24 h-24 animate-scale-in">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center shadow-2xl animate-scale-pulse">
                <Moon className="h-10 w-10 text-white animate-moon-glow" fill="currentColor" />
              </div>
            ...
            <div className="space-y-2 opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.2s" }}>
              ...
              <p className="text-foreground/80 font-medium opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.3s" }}>
                আপনার ঈদি পেয়ে খুশি হলাম
              </p>
            </div>

            {/* Thank You Box with glow */}
            <div className="bg-primary/5 rounded-2xl p-5 border border-primary/10 opacity-0 animate-slide-up fill-forwards hover:border-primary/30 transition-all duration-500 hover:shadow-lg hover:shadow-primary/10" style={{ animationDelay: "0.4s" }}>
            ...
            <div className="opacity-0 animate-scale-in fill-forwards" style={{ animationDelay: "0.5s" }}>
              ...
              <p className="text-foreground/50 text-sm mt-4 opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.6s" }}>
                - {paymentInfo.ownerName}
              </p>
            </div>

            {/* Hearts with enhanced animation */}
            <div className="flex justify-center gap-4 pt-2 opacity-0 animate-slide-up fill-forwards" style={{ animationDelay: "0.7s" }}>
